...
async function main() {
	...
	const AlbumContract = await ethers.getContractFactory('AlbumArt');
	const albumArt = await AlbumContract.deploy();
	await albumArt.deployed();
	saveFrontendFiles(albumArt, 'AlbumArt');
	console.log('AlbumArt deployed to:', albumArt.address);
}
